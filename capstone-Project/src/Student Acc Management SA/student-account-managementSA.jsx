...
function StudentAccountManagementSA() {
    ...
    {/* Styled confirmation dialog used by every destructive action here
         (single delete, batch delete, batch archive, status toggle). */}
    ...
    async function loadData() {
        ...
        setLoadError('');
        try {
            const [studentRows, sectionRows] = await Promise.all([fetchStudents(), fetchSections()]);
    ...
    {/* Batch import textarea + section to assign every row to + any parse errors */}
    ...
    function openInsertModal(student = null) {
        ...
        // Blank on edit = "keep current password"; only set to reset it.
    ...
    {/* Exports whatever is currently on screen (respects the active
         section filter + search), so the file always matches the
         registered students actually shown. */}
    function downloadClassListCSV() {
        const escapeCsv = (value) => {
            const str = String(value ?? '');
            return /[",\n]/.test(str) ? `"${str.replace(/"/g, '""')}"` : str;
        };
        const header = ['ID Number', 'Name', 'Section', 'Email Address', 'Status'];
        const lines = [header.join(',')];
        filteredStudents.forEach((s) => {
            lines.push([s.idNum, s.fullName, s.section || '', s.email || buildEmailPreview(s.firstName, s.lastName), s.isActive ? 'Active' : 'Inactive'].map(escapeCsv).join(','));
        });
        const csvContent = lines.join('\r\n');
        const blob = new Blob([csvContent], { type: 'text/csv;charset=utf-8;' });
        const url = URL.createObjectURL(blob);
        const activeSection = sections.find((s) => s.section_id === sectionFilter);
        const label = (activeSection?.section_name || 'all-sections').replace(/[^a-z0-9-]+/gi, '-');
        const link = document.createElement('a');
        link.href = url;
        link.download = `${label}-class-list-${new Date().toISOString().slice(0, 10)}.csv`;
    ...
    // Section + Search (name / ID), combined. Both narrow the same
    // already-loaded `students` array — no extra fetch involved.
    ...
    {/* --- Multi-select + batch actions ---
         `filteredStudents` drives the header "select all" so it only ticks
         what's on screen; the full `students` array keeps a row selected if
         a search or section filter temporarily hides it. */}
    ...
    async function handleBatchSubmit(event) {
        ...
        if (!batchSectionId) {
            setBatchError('Choose a section to assign these students to.');
            return;
        }
                                    ...
                                    <table className="styled-table text-center-table" style={{ marginTop: '10px' }}>
                                        <thead>
                                            <tr>
                                                ...
                                                <th>ID Number</th><th>Name</th><th>Section</th><th>Email Address</th><th>Status</th><th>Action</th>
                                            </tr>
                                        </thead>
                                        <tbody>
                                            {isLoadingStudents && (
                                                <tr><td colSpan={7} style={{ textAlign: 'center', padding: '24px' }}>Loading students…</td></tr>
                                            )}
                                            ...
                                            {!isLoadingStudents && !loadError && students.length === 0 && (
                                                <tr><td colSpan={7} style={{ textAlign: 'center', padding: '24px' }}>No students yet.</td></tr>
                                            )}
                                            {!isLoadingStudents && !loadError && students.length > 0 && filteredStudents.length === 0 && (
                                                <tr><td colSpan={7} style={{ textAlign: 'center', padding: '24px' }}>No students match your search/filter.</td></tr>
                                            )}
                                    ...
                                    <div className="modal-two-col">
                                        ...
                                        <div style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: '16px' }}>
                                            ...
                                            <div className="field-group">
                                                ...
                                                <select name="sectionId" className="modal-input" required defaultValue={editingStudent?.sectionId || ""}>
                        ...
                        <div className="modal-box" style={{ maxWidth: '700px' }}>
                            ...
                            <form onSubmit={handleBatchSubmit}>
                                <div className="modal-body">
                                    <div className="field-group">
                                        <label className="field-label">Assign Section</label>
                                        <select className="modal-input" value={batchSectionId} onChange={(e) => { setBatchSectionId(e.target.value); setBatchError(''); }} disabled={isImporting} required>
                                            <option value="" disabled>Select Section</option>
                                            {sections.map(s => (
                                                <option key={s.section_id} value={s.section_id}>{s.section_name}</option>
                                            ))}
                                        </select>
                                        <span style={{ fontSize: '12px', color: '#6b7280' }}>Every student in this batch is assigned to the same section.</span>
                                    </div>
                                    <div className="field-group">
                                        ...
                                        />
                                        {batchError && (
                                            <span style={{ fontSize: '13px', color: batchResult ? '#b45309' : '#dc2626', whiteSpace: 'pre-line' }}>{batchError}</span>
                                        )}
                                ...
                                <div className="modal-footer">
                                    ...
                                    <button type="submit" className="btn-modal-submit" disabled={isImporting || !batchText.trim() || !batchSectionId}>
